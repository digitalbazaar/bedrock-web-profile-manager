...
export class ProfileManager {
   ...
   * @param {string} [options.useCache=false] - Return cached value if set,
   *   otherwise get fresh copy and update the cache.
   *
  ...
  async getProfileIds({useCache = false} = {}) {
    const {accountId} = this;
    const profileAgentRecords = await this._getProfileAgentRecords(
      {accountId, useCache});
    return profileAgentRecords.map(({profileAgent}) => profileAgent.profile);
  }
  ...
  _getCache(key) {
    ...
    const newCache = new LRU({max: 1000});
